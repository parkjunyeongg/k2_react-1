...
const Taccident = () => {
    const apiData = {
      ...
      //배열의 map
      let newv ;
      newv = objk.map((k)=>apiData[k]) // = Object.values
      console.log("새배열",newv)
      
      //배열의 filter
      let data;
      data = objk.filter((item)=> item=='data');
      data= data.map((k)=>apiData[k]);
      console.log("filter로 추출",data);

      //오브젝트 키로 접근
      data = apiData.data;
      console.log("오브젝트 키 data추출1",data)
      data = apiData['data'];
      console.log("오브젝트 키 data추출2",data)

      //대분류 추출
      let c1 = data.map((item) => item.사고유형_대분류 );
      console.log("대분류 추출 1",c1);
      c1=new Set(c1);
      console.log("대분류 추출 2 Set으로 중복 제거",c1);
      c1 = [...c1];
      console.log("대분류 추출 3 Set을 배열로 변환",c1);
